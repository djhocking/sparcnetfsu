...
density_summary_session <- d_preds %>%
  group_by(date) %>%
  dplyr::select(date, pp, estimate) %>%
  dplyr::summarise(pp = mean(pp),
                   mean = mean(estimate),
                   sd = sd(estimate))

density_summary <- d_preds %>%
...
The mean density is over time and space is `r round(mean(d_preds$estimate), 2)` $\pm$ `r round(sd(d_preds$estimate), 2)`.
...
g3 <- ggplot(s_preds, aes(x=pp, y=estimate, color = sex, group=sex)) +
  geom_line(size = 1, position = position_dodge(width=0.5)) +
  geom_errorbar(aes(ymin=lwr,ymax=upr), width=0, size=0.75, color=1,
                position = position_dodge(width=0.5)) +
  geom_point(size=4, position = position_dodge(width=0.5)) +
...
ggsave("analysis/figures/sigma_session.pdf", plot = g3)

g4 <- ggplot(s_preds, aes(x=pp, y=hr90_estimate, color = sex, group=sex)) +
  geom_line(size = 1, position = position_dodge(width=0.5)) +
  geom_errorbar(aes(ymin=hr90_lwr,ymax=hr90_upr), width=0, size=0.75, color=1,
                position = position_dodge(width=0.5)) +
  geom_point(size=4, position = position_dodge(width=0.5)) +
...
ggsave("analysis/figures/home_range_session.pdf", plot = g4)
...
g6 <- ggplot(p_preds, aes(x = (rain * stds[which(stds$var == "rain"), "sd"]) + stds[which(stds$var == "rain"), "mean"], y = estimate, color = soil, group = soil)) +
...
ggsave("analysis/figures/detection_ain_soil.pdf", plot = g6, width = 8, height = 5)
...
g7 <- ggplot(p_preds, aes(x = (soil * stds[which(stds$var == "soil"), "sd"]) + stds[which(stds$var == "soil"), "mean"], y = estimate, color = rain, group = rain)) +
 geom_ribbon(aes(ymin=lwr,ymax=upr, fill = rain), alpha=0.1) +
geom_line(size=1.5) +
  theme_bw() + # scale_color_fivethirtyeight() +
  xlab("Soil Temperature (C)") + ylab("Initial Capture Prob.")

ggsave("analysis/figures/detection_soil_rain.pdf", plot = g7, width = 8, height = 5)
